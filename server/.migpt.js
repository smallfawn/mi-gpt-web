...
`.trim();
const speakerConfig = require("./speaker.json")
module.exports = {
  systemTemplate,
  bot: {
    name: "傻妞",
  ...
  speaker: {
    ...
    // 小米 ID
    userId: speakerConfig.userId, // 注意：不是手机号或邮箱，请在「个人信息」-「小米 ID」查看
    // 账号密码
    password: speakerConfig.password,
    // 小爱音箱 DID 或在米家中设置的名称
    did: speakerConfig.did, // 注意空格、大小写和错别字（音响 👉 音箱）
    ...
    callAIKeywords: speakerConfig.callAIKeywords,
    // 当消息以下面的关键词开头时，会进入 AI 唤醒状态
    wakeUpKeywords: speakerConfig.wakeUpKeywords,
    // 当消息以下面的关键词开头时，会退出 AI 唤醒状态
    exitKeywords: speakerConfig.exitKeywords,
    // 进入 AI 模式的欢迎语
    onEnterAI: speakerConfig.onEnterAI, // 设为空数组时可关闭提示语
    // 退出 AI 模式的提示语
    onExitAI: speakerConfig.onExitAI, // 为空时可关闭提示语
    // AI 开始回答时的提示语
    onAIAsking: speakerConfig.onAIAsking, // 为空时可关闭提示语
    // AI 结束回答时的提示语
    onAIReplied: speakerConfig.onAIReplied, // 为空时可关闭提示语
    // AI 回答异常时的提示语
    onAIError: speakerConfig.onAIError, // 为空时可关闭提示语
    ...
    ttsCommand: speakerConfig.ttsCommand,
    // 设备唤醒指令，请到 https://home.miot-spec.com 查询具体指令
    wakeUpCommand: speakerConfig.wakeUpCommand,
    ...
    streamResponse: speakerConfig.streamResponse,
    // 连续对话时，无响应多久后自动退出
    exitKeepAliveAfter: speakerConfig.exitKeepAliveAfter, // 默认 30 秒，建议不要超过 1 分钟
    // 连续对话时，下发 TTS 指令多长时间后开始检测设备播放状态（默认 3 秒）
    checkTTSStatusAfter: speakerConfig.checkTTSStatusAfter, // 当小爱长文本回复被过早中断时，可尝试调大该值
    // 连续对话时，播放状态检测间隔（单位毫秒，最低 500 毫秒，默认 1 秒）
    checkInterval: speakerConfig.checkInterval, // 调小此值可以降低小爱回复之间的停顿感，请酌情调节
    ...
    // 是否启用调试
    debug: speakerConfig.debug, // 一般情况下不要打开
    // 是否跟踪 Mi Service 相关日志（打开后可以查看设备 did）
    enableTrace: speakerConfig.enableTrace, // 一般情况下不要打开
    // 网络请求超时时长（单位毫秒，默认 5 秒）
    timeout: speakerConfig.timeout,
  },
};